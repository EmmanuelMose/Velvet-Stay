...
export const getAllCustomerSupportTicketsController = async (req, res) => {
    try {
...
export const getCustomerSupportTicketByIdController = async (req, res) => {
    try {
...
export const updateCustomerSupportTicketController = async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ message: "Invalid Ticket ID" });
...
export const deleteCustomerSupportTicketController = async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ message: "Invalid Ticket ID" });
